...
def load_test_case(yaml_file: str) -> list[tuple]:
    ...
    list_cases = []
    for case in raw_test_case["cases"]:
        ...
        list_cases.append((request, webhook_config, case["expected_response"]))
...
@pytest.mark.parametrize(("req", "webhook_config", "expected_response"),
                         load_test_case(os.path.join(HTTP_SERVER_TEST_DATA_DIR, "test_case_1.yaml")))
def test_http_server(req, webhook_config, expected_response, tmp_path):
...
def test_auto_reload(tmp_path):
    ...
    with open(webhook_config_file, "w") as f:
        yaml.safe_dump(list_cases[0][1], f)
    ...
    for req, webhook_config, expected_response in list_cases:
        with open(webhook_config_file, "w") as f:
            yaml.safe_dump(webhook_config, f)
...
def test_two_webhooks_same_server(tmp_path):
    ...
    for req, _, expected_response in list_cases:
        url = f"http://localhost:{port}{req['path']}"
        response = requests.post(url, json=req["body"])
        json_response = json.loads(response.content.decode("utf-8"))
